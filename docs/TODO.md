...
**철학**: 요코이 군페이의 "성숙한 기술의 수평적 사고". 동영상을 파일로 호스팅하지 않고, 클라이언트가 실시간으로 그려내는 "레시피"로 저장한다. 무료 서버(Supabase 1GB)에서도 운영 가능한 동영상 플랫폼. 화질 트레이드오프를 결함이 아닌 양식으로 선언 — 제약이 곧 정체성.

**핵심 발상**: 영상 = 픽셀 데이터가 아니라 영상 = 재생 명령(레시피). 서버는 레시피만 저장, 클라이언트가 Canvas/WebGL로 렌더링.

**접근 축 2가지**:

1. **인덱스드 패턴 방식** — 프레임을 제한 팔레트(그레이스케일/인덱스 컬러)로 양자화, 반복 패턴을 타일 사전화. "타일 ID + 팔레트 인덱스 + 프레임 델타"로 압축. SNES/GBA 타일맵 구조의 동영상 적용
2. **벡터 트레이싱 방식** — 프레임의 윤곽/영역을 벡터 패스로 추출, 프레임 간 패스 변형 델타만 저장. SVG급 해상도 독립성
...
**조사 필요 항목**:
- [ ] 프레임 → 인덱스드 타일 변환 파이프라인 실현 가능성 + 기존 라이브러리
- [ ] 프레임 → 벡터 트레이싱 자동화 품질 (potrace, vtracer 등)
- [ ] 레시피 포맷 설계 — 프레임 간 델타 인코딩 효율
- [ ] Canvas/WebGL 실시간 렌더링 성능 (모바일 포함)
- [ ] 1분 영상 기준 레시피 용량 추정
- [ ] 오디오 처리 — 영상과 별도로 Opus/AAC 경량 인코딩 or 칩튠 변환
- [ ] 기존 유사 시도 조사 — 선행 사례, 한계점, 차별화 가능성
