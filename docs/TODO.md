...
- [ ] 모험 일지 — OS 내 폴더에 개발자(크리에이터)의 실제 여정 일지를 배치. "왜 이걸 해야 하는가"의 답. 들어가면 바로 보이지만 읽어야 맥락이 생기는 구조. 일지를 읽은 유저가 호기심과 동기를 얻어 FeTreeGO! 수수께끼에 참여하게 되는 퍼널. 일지 자체가 콘텐츠 — "어느 방탕한 모험가의 여정"을 따라가는 경험. OS 업데이트마다 새 일지 추가 → 새 수수께끼 연동
- [ ] 커스텀 별자리 — 유저가 수집한 스탬프 간 선분을 직접 이어 자기만의 별자리를 만드는 기능. 수집 보상이 단순 완료 표시가 아니라 창작으로 이어짐. 스탬프 수가 충분히 늘어난 후 도입. 저장: localStorage → 추후 FeTreeOS 파일 시스템 연동
...
### 저장소 아키텍처 — 서버 vs 유저 데이터 분리

**원칙**: 서버(콘텐츠)와 유저(에셋/작업물)의 저장소를 분리. 유저 데이터는 사용자 소유 계정에.

```
서버(콘텐츠) — Supabase
  ├─ 레시피 영상, 공유 에셋, 앱 자체
  ├─ assets/user_assets 테이블 (참조 구조)
  └─ 서비스가 제공하는 것
...
**저장소 백엔드 (교체 가능 추상화)**:
- Supabase Storage: 서비스 콘텐츠 기본 (무료 1GB)
- GitHub: OAuth + REST API, 버전 히스토리 무료, 심사 불필요
- Google Drive: OAuth + drive.file 스코프, 앱 전용 폴더 생성 가능, 15GB 무료
- 로컬 File System Access API: 오프라인 폴백

**분리 근거**:
- 서비스 비용 절감 (유저 바이너리가 서버 스토리지를 먹지 않음)
- 스케일링 (유저 증가 ≠ 서버 스토리지 비용 비례 증가)
- 사용자 소유권 (웹3적 접근, 블록체인 없이. 서비스 소멸해도 파일 잔존)

**인증 & 온보딩**:
- 1회 OAuth → refresh token 저장 → 이후 자동 갱신
- 게스트 모드: Supabase 임시 버킷 (TTL 3~7일) → 계정 연결 시 마이그레이션

**iframe ↔ 호스트 브릿지**:
- postMessage 프로토콜: iframe 앱이 호스트에 save/load 요청
- 호스트가 저장소 백엔드 연동 담당, iframe은 프로토콜만 알면 됨

- [ ] postMessage 프로토콜 스펙 설계
- [ ] 저장소 추상화 레이어 (Supabase/GitHub/Drive 교체 가능)
- [ ] 게스트 모드 (임시 UUID + TTL)
- [ ] OAuth 연동 (GitHub 우선, Google Drive 후순위)
- [ ] 가상 FS ↔ 원격 저장소 동기화 로직
...
### 레시피 영상 플랫폼 — 枯れた技術の水平思考

**철학**: 요코이 군페이(横井軍平)의 "성숙한(枯れた) 기술의 수평적 사고". 동영상을 파일로 호스팅하지 않고, 클라이언트가 실시간으로 그려내는 "레시피"로 저장한다. 무료 서버(Supabase 1GB)에서도 운영 가능한 동영상 플랫폼. 화질 트레이드오프를 결함이 아닌 양식으로 선언 — 제약이 곧 정체성.

**핵심 발상**: 영상 = 픽셀 데이터가 아니라 영상 = 재생 명령(레시피). 서버는 레시피만 저장, 클라이언트가 Canvas/WebGL로 렌더링.

**접근 축 2가지**:

1. **인덱스드 패턴 방식** (픽셀 레시피) — 프레임을 제한 팔레트(그레이스케일/인덱스 컬러)로 양자화, 반복 패턴을 타일 사전화. "타일 ID + 팔레트 인덱스 + 프레임 델타"로 압축. SNES/GBA 타일맵 구조의 동영상 적용
2. **벡터 트레이싱 방식** — 프레임의 윤곽/영역을 벡터 패스로 추출, 프레임 간 패스 변형 델타만 저장. SVG급 해상도 독립성. 모자란 애니메이트(내장 벡터 애니메이션 도구)로 만든 콘텐츠는 변환 없이 네이티브 레시피

**미감**: 도트 애니메이션이 픽셀 부족에서 태어나 독립 양식이 된 것처럼, 이 포맷 고유의 시각 스타일이 플랫폼 정체성. "화질이 나쁜 플랫폼"이 아니라 "여기서만 볼 수 있는 화면"

**FeTreeOS 연계**: 레트로 OS 메타포와 자연스럽게 맞물림. 내장 앱 또는 iframe 앱으로 탑재

**기술 조사 결과 (2026-04-30)**:

픽셀 패턴화(인덱스드 타일 방식) 실현 가능성 확인:
- Color quantization (Median Cut, Octree) — 수십 년 검증된 기술, JS 라이브러리 존재 (RgbQuant.js)
- 브라우저 실시간 변환 선행 사례 존재 (video-to-pixel-art: WebGL + Canvas)
- 타일 단위 팔레트 최적화 데모 존재 (Tiled Palette Quantization)
- 변환 파이프라인: 원본 -> ffmpeg.wasm 프레임 추출 -> 팔레트 양자화 -> 타일 분할/사전화 -> 델타 인코딩 -> 레시피 출력
- 서버 비용 0원 (변환도 클라이언트 WASM으로 처리)

**비용 구조 핵심**: 타일 사전 상한이 포맷의 "해상도"를 결정.
- 256종 = 게임보이급, 1024종 = SNES급, 4096종 = GBA급
- 팔레트 색수(4/16/256)는 그 다음 레버
- 고정 비용: 타일 사전 (영상 복잡도 비례) / 가변 비용: 프레임 델타 (움직임량 비례)

**용량 추정**:
- 극단 압축 (160x120, 4색, 10fps, 타일 256종): ~34KB/분
- 중간 (320x240, 16색, 15fps, 타일 512종): ~5.4MB/분
- Supabase 1GB 기준: 극단은 3만 편, 중간은 180편

**다음 단계 — 납득 가능 선 탐색**:
- [ ] 이미지 1장 변환 프로토타입 (HTML+Canvas+JS, 팔레트 수/타일 크기 슬라이더)
- [ ] "납득 가능한 최소 비용" 기준선 확정 (눈으로 보고 판단)
- [ ] 기준선을 프레임으로 확장 -> 델타 인코딩 붙이기
- [ ] 파이프라인 완성 (변환기 + 레시피 포맷 + 플레이어)
- [ ] 오디오 처리 — Opus/AAC 경량 인코딩 or 칩튠 변환

**아이디어 노트 — 인상 레시피 (바우링걸 방식)**:
영상을 "보는" 레시피가 아니라 "읽는" 레시피. 색 분포/움직임/밝기/음량에서 감정 곡선을 추출, 이모티콘+컬러바 타임라인으로 치환. 용량 수 바이트~수 KB. 타일 레시피의 메타/프리뷰 레이어, 또는 극단적 압축의 또 다른 양식으로 기용 가능.
